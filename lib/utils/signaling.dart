...
import 'package:streaming_test_null_safety/utils/ApiClient.dart';
import 'package:streaming_test_null_safety/utils/RequestConfig.dart';
import 'package:streaming_test_null_safety/utils/rtc_ice_candidate.dart';
...
class Signaling {
  Signaling(this._url, this._secret, this._userName, this._iceServer)
      : _iceServers = {
          'iceServers': [
            ...
            {"urls": "stun:$_iceServer"},
            {
              'urls': [
                "turn:$_iceServer",
                "turn:$_iceServer?transport=tcp",
                "turn:$_iceServer?transport=udp"
              ],
  ...
  Future<String?> createWebRtcSession({String? sessionId}) {
  ...
  Future<String?> createWebRtcToken({required String sessionId}) {
    final Map<String, dynamic> bodyMap = <String, dynamic>{
      'session': sessionId,
      'role': JsonConstants.publisher,
      'data': ''
    };
    final Map<String, dynamic> headersMap = <String, dynamic>{
      'Authorization':
    ...
    return ApiClient()
        .request<Map<String, dynamic>>(Config(
            ...
            body: RequestBody.json(bodyMap),
            method: RequestMethod.post,
            responseType: ResponseBody.json())).then((Map<String, dynamic>? jsonResponse) {
      ...
      _createLocalPeerConnection().then((_) {
        _idJoinRoom = _sendJson(JsonConstants.joinRoom, {
          JsonConstants.metadata: "{\"clientData\": \"$_userName\","
              ...
              "\"hasVideo\": \"true\"}",
          'secret': '',
          'platform': Platform.isAndroid ? 'Android Browser' : 'iOS',
          // 'dataChannels': 'false',
          'session': session,
          'token': token,
          'recorder': true
